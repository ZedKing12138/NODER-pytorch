...
def boundary_loss(df):
    return (df[:,0,0,:,:]**2 + df[:,1,0,:,:]**2 + df[:,2,0,:,:]**2 + \
           df[:,0,img_size[0]-1,:,:]**2 + df[:,1,img_size[0]-1,:,:]**2 + df[:,2,img_size[0]-1,:,:]**2).mean() + \
           (df[:,0,:,0,:]**2 + df[:,1,:,0,:]**2 + df[:,2,:,0,:]**2 + \
           df[:,0,:,img_size[1]-1,:]**2 + df[:,1,:,img_size[1]-1,:]**2 + df[:,2,:,img_size[1]-1,:]**2).mean() + \
           (df[:,0,:,:,0]**2 + df[:,1,:,:,0]**2 + df[:,2,:,:,0]**2 + \
           df[:,0,:,:,img_size[2]-1]**2 + df[:,1,:,:,img_size[2]-1]**2 + df[:,2,:,:,img_size[2]-1]**2).mean() 

#MSE损失，用于在每个epoch后简单地评估配准的效果
def MSE(warped,target):
    return torch.mean((target-warped)**2)
